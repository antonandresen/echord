...
import { GatewayOpCodes, GatewayDispatchEvents } from '../types/gateway';
...
export class GatewayClient extends EventEmitter {
  ...
  public connect(url: string = 'wss://gateway.discord.gg'): void {
    this.ws = new WebSocket(`${url}/?v=10&encoding=json${this.compress ? '&compress=zlib-stream' : ''}`);
  ...
  private handleDispatch(event: string, data: unknown): void {
    switch (event) {
      case GatewayDispatchEvents.Ready:
        if (typeof data === 'object' && data && 'session_id' in data && 'resume_gateway_url' in data) {
